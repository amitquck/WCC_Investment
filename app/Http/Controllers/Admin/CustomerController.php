<?php
	...
	use App\Exports\HoldCustomerReportExport;
	use App\Exports\CustomerPDCIssueReportExport;
...
use App\Model\DirectAssociateCommission;
	 ...
	 //        }
		...
		/*  ** Important!! **
			...
			** Insert Direct Associate Commission for 1st Introducer Total Investment(client) **
			...
			$associates = AssociateCommissionPercentage::where('no_of_introducer',1)->orderBy('start_date')->get();
			foreach($associates as $key => $associate){
				$delete_DAC_customer = DirectAssociateCommission::where('customer_id',$associate->customer_id)->delete();
			  	...
			  	$direct_associate_commission->total_withdraw = $associate->customertotalwithdraw($associate->customer_id);
			...
			// $comm_table = AssociateCommissionPercentage::get();
			// foreach($comm_table as $key =>$comm){
			// 	$txn = CustomerTransactions::where('customer_id',$comm->customer_id)->orderBy('deposit_date')->first();
			// 	dd($txn);
			// }
			...
			// dd($cust_txn);
	      	if(\Auth::user()->login_type == 'superadmin' || \Auth::user()->login_type == 'employee'){
		        ...
		        if($request->customer_name != null && $request->has('customer_name') ){
		        ...
		        }else if($request->state_id != null && $request->has('state_id') && $request->city_id == 'Select City'){
			        $users->whereHas('customerdetails',function($q) use ($request){
		                return $q->where('state_id',$request->state_id);
		            });
		        }else if($request->state_id != null && $request->city_id != null && $request->has('city_id')){
		            $users->whereHas('customerdetails',function($q) use ($request){
		                return $q->where('state_id',$request->state_id)->where('city_id',$request->city_id);
		            ...
		            $users->whereHas('customertransactions',function($q) use ($request){
		                return $q->select('*',DB::raw('SUM((amount) * ( CASE WHEN cr_dr = "cr" THEN 1 WHEN transaction_type = "withdraw" THEN -1 END )) AS cust_balance'))->groupBy('customer_id')->having('cust_balance',$request->customer_deposit);
		            });
		        }


		        // else if($request->customer_balance != null && $request->has('customer_balance')){
		        //     $users->whereHas('customertransactions',function($q) use ($request){
		        //         return $q->select('customer_id',DB::raw('SUM((amount) * ( CASE WHEN transaction_type = "deposit" THEN 1 WHEN transaction_type = "withdraw" THEN -1 END )) AS top_customers'))->groupBy('customer_id');
		...
		public function store(Request $request){
			...
			try{
				...
				// dd($customer);
				$name = '';
				//dd($request->file('image'),$request->nominee_image);
				 //dd($request->image,$request->nominee_image);
		        if($request->has('image')){
		           ...
		           $nominee_name ='';
			        ...
			        if($request->has('nominee_image')){
                    $nominee_image = $request->file('nominee_image');
                    $nominee_name = uniqid().'.'.$nominee_image->getClientOriginalExtension();

                    $ndestinationPath = public_path('images/customer');
                    $nominee_image->move($ndestinationPath, $nominee_name);

                }

		        //dd($name,$nominee_name);
		        ...
		        $activity_log->save();//account_holder_name

                if(!empty($request->account_holder_name)){
                    foreach($request->account_holder_name as $key => $acc_holder_name){
                        $customer->customerBankDetail()->create([
                            'account_holder_name' => $acc_holder_name?$acc_holder_name:NULL,
                            'bank_name' => $request->bank_name[$key]?$request->bank_name[$key]:NULL,
                            'account_number' => $request->account_number[$key]?$request->account_number[$key]:NULL,
                            'ifsc_code' => $request->ifsc_code[$key]?$request->ifsc_code[$key]:NULL,
                            'pan_no' => $request->pan_no[$key]?$request->pan_no[$key]:NULL,
                            'created_by' => $user_id,
                        ]);
                    }
                }
				...
				foreach($request->cheque_issue_date as $key => $issue_date){
					// if($issue_date != null){
						...
						$customer->customersecuritycheque()->create([
							'cheque_issue_date' => $issue_date?date('Y-m-d',strtotime($issue_date)):NULL,
							'cheque_maturity_date' =>$request->cheque_maturity_date[$key]?date('Y-m-d',strtotime($request->cheque_maturity_date[$key])):NULL,
							...
							'cheque_date' => isset($request->cheque_date[$key])?date('Y-m-d',strtotime($request->cheque_date[$key])):NULL,
				...
				$customer->customerinterestpercents()->create([
					...
					'start_date' => date('Y-m-d',strtotime($request->cust_apply_date)),
					'active_status' => 1,
					'created_by' => $user_id,
				]);
					...
					foreach($request->associate_name as $key => $value){
						if($value>0){
							$customer->associatecommissions()->create([
								...
								'start_date' => date('Y-m-d',strtotime($request->applicable_date[$key])),
		...
		public function edit(Request $request,$id){
			...
			$banks = CompanyBank::get();
			$associates = User::where('login_type','associate')->get();
			$isCustomer = CustomerTransactions::where('customer_id', $customer->id)->first();
			$reward = $record = null;
			if($isCustomer != null){
				$reward = CustomerReward::where('customer_id',$customer->id)->first();
				$cus_txn = CustomerTransactions::where('customer_id', $customer->id)->where('transaction_type','deposit')->orderBy('deposit_date')->first();
				$record = CustomerReward::where('month', '>=', Carbon::parse($cus_txn->deposit_date)->format('m'))->where('year', '>=',Carbon::parse($cus_txn->deposit_date)->format('Y'))->first();

			}

			return view('admin.customers.edit',compact('customer','countries','states',
				'cities','banks','security_cheque','associates','reward','record'));
		}

		public function update(Request $request){
			// dd(date('Y-m-d',strtotime($request->dob)));
			//  dd($request->all());
			 // dd($request->scan_copy);
			$id = $request->id;
			$this->validate($request,[
				'name' => 'required',
			...
			if($request->isMethod('post')){
				...
				if($customer->count() > 0){
					...
					// $customer->status = 1;//Don't Uncomment.
					$customer->save();
					// $uploaded = true;
					// dump($customer->customerdetails());
					// if($customer->customerdetails()->count() > 0){
					$name = $customer->customerdetails->image;
			        if($request->has('image')!=null){
                     ...
                     $nominee_name =$customer->customerdetails->nominee_image;
                     //dd($nominee_image);
					...
					$customer->customerdetails()->update([
						...
						'nominee_age' => $request->nominee_age,
						'nominee_sex' => $request->nominee_sex,
						'nominee_dob' => $request->dob?date('Y-m-d',strtotime($request->dob)):'',
						'nominee_relation_with_applicable' => $request->nominee_relation_with_applicable,
						'nominee_address_one' => $request->nominee_address_one,
						'nominee_address_two' => $request->nominee_address_two,
						'nominee_city_id' => $request->nominee_city_id,
                    ...
                    if(!empty($request->account_holder_name)){
                        foreach($request->account_holder_name as $key => $acc_holder_name){
                            $per_bank_customer = CustomerBankDetail::where('customer_id',$request->bank_customer_id[$key])->where('id',$request->bank_table_ids)->first();
                                $per_bank_customer->account_holder_name = $acc_holder_name?$acc_holder_name:NULL;
                                $per_bank_customer->bank_name = $request->bank_name[$key]?$request->bank_name[$key]:NULL;
                                $per_bank_customer->account_number = $request->account_number[$key]?$request->account_number[$key]:NULL;
                                $per_bank_customer->ifsc_code = $request->ifsc_code[$key]?$request->ifsc_code[$key]:NULL;
                                $per_bank_customer->pan_no = $request->pan_no[$key]?$request->pan_no[$key]:NULL;
                                $per_bank_customer->updated_by = auth()->user()->id;
                                $per_bank_customer->save();
                        }
                    }

                    if(!empty($request->account_holder_names)){
                        foreach($request->account_holder_names as $key => $acc_holder_names){
                            $customer->customerBankDetail()->create([
                                'account_holder_name' => $acc_holder_names?$acc_holder_names:NULL,
                                'bank_name' => $request->bank_names[$key]?$request->bank_names[$key]:NULL,
                                'account_number' => $request->account_numbers[$key]?$request->account_numbers[$key]:NULL,
                                'ifsc_code' => $request->ifsc_codes[$key]?$request->ifsc_codes[$key]:NULL,
                                'pan_no' => $request->pan_nos[$key]?$request->pan_nos[$key]:NULL,
					...
					if($request->cheque_issue_date){//$request->cheque_issue_dates ||
						foreach($request->ids as $key => $id){
					        ...
					        if($per_customer){
								$per_customer->cheque_issue_date = isset($request->cheque_issue_date[$key])?date('Y-m-d',strtotime($request->cheque_issue_date[$key])):NULL;
								$per_customer->cheque_maturity_date = isset($request->cheque_maturity_date[$key])?date('Y-m-d',strtotime($request->cheque_maturity_date[$key])):NULL;
								...
								$per_customer->cheque_date = isset($request->cheque_date[$key])?date('Y-m-d',strtotime($request->cheque_date[$key])):NULL;
								if($request->has('scan_copy.'.$key)){
					...
					if($request->cheque_issue_dates){
						foreach($request->cheque_issue_dates as $key => $issue_date){
							...
							$customerscan = '';
					        if($request->has('scan_copys.'.$key)!=null){
							...
							$customer->customersecuritycheque()->create([
								'cheque_issue_date' => isset($issue_date)?date('Y-m-d',strtotime($issue_date)):NULL,
								'cheque_maturity_date' => isset($request->cheque_maturity_date[$key])?date('Y-m-d',strtotime($request->cheque_maturity_date[$key])):NULL,
		...
		public function destroy(Request $request){
			...
			if($user){
				...
				CustomerSecurityCheque::where('customer_id',$request->customer_id)->delete();
				CustomerBankDetail::where('customer_id',$request->customer_id)->delete();
				AssociateHeirarchy::where('customer_id',$request->customer_id)->delete();
				AssociateRewardTemp::where('customer_id',$request->customer_id)->delete();
				AssociateTransactionTemp::where('customer_id',$request->customer_id)->delete();
				CustomerRewardTemp::where('customer_id',$request->customer_id)->delete();
				CustomerTransactionTemp::where('customer_id',$request->customer_id)->delete();
				DirectAssociateCommission::where('customer_id',$request->customer_id)->delete();
    ...
    {
        $commrates = AssociateCommissionPercentage::where('customer_id',$request->cust_id)->where('associate_id',$request->ass_id)->get();
        return view('admin/customers/associate_all_commissions',compact('commrates'));
    }
	...
	public function customerAssociateCommission(Request $request,$customerId){
		...
		// 	dd('das');
		// }
		$investhist = CustomerInterestPercentage::where('customer_id',decrypt($customerId))->get();
        return view('admin/customers/add_customer_associate_percent',['customer' => $customer,'associate'=>$associate,'customer_interest_percent'=>$customer_interest_percent,'investhist'=>$investhist]);
	}
	...
	public function investmentStore(Request $request,$id)
	{
        ...
        $generated_payout = CustomerReward::whereRaw("CONCAT(customer_rewards.year,customer_rewards.month) >= $year_month")->first();
        // dd($generated_payout);
        if($generated_payout){
        ...
        if($request->payment_type != 'cash'){
			...
			$txnEntry->respective_table_name = 'customer_transactions';
	...
	public function commissionStore(Request $request,$id){
		...
		if($request->sum_of_commission > 36|| $request->sum_of_commission > 36){
		...
		if($request->associate_name != NULL || $request->associate_names != NULL){
			...
			if($request->associate_names != NULL && sizeof(array_filter($request->associate_names))){
				foreach($request->associate_names as $keys => $values){
					if($values>0){
						...
						$asso_comm_per->start_date  = date('Y-m-d',strtotime($request->applicable_date[$keys]));
            ...
            associateHeirarchy($customer->id);
			return redirect()->back()->with('success',$customer->name. ' Successfully Add Commission');
		}
	...
	public function payoutGenerates(Request $request)
	{
		...
		$customers = User::whereHas('customertransactions',function($q) use ($year,$month){
			// dd($q->with('customerdetail'));->where('customer_id',501)
		 $q->whereIn('customer_id', function($query){
				 $query->select('customer_id')->from('customer_details')->where('payment_type', '!=', 'no_interest')->get();
			})->where('deposit_date', '<=', Carbon::parse($year.'-'.$month)->format('Y-m-t'));
		})->where('lock_status',0)->whereLoginType('customer')->get();
		// dd($customers);
		// ->where('id',504)
	...
	protected function calculateCustomerPayout($customer,$month,$year,$postingDate){
		...
		$customer_transactions = CustomerTransactions::where('deposit_date','>=',Carbon::parse($year.'-'.$month.'-01')->format('Y-m-d'))->where('deposit_date','<=',Carbon::parse($year.'-'.$month.'-01')->format('Y-m-t'))->where('customer_id',$customer->id)->orderBy('deposit_date')->get();
			//$customer->id
		// dd($customer_transactions);
		if($customer_transactions->count()){
			$previous_txn_type = $previous_txn_date =  NULL;
			foreach($customer_transactions as $customer_transaction){
				...
				if($customer_transaction->transaction_type == 'deposit' || ($previous_txn_type == 'deposit' && $previous_txn_date == $customer_transaction->deposit_date && $customer_transaction->transaction_type == 'withdraw')){//$balance > 0 &&
				...
				// dump($start_date.' start_up');
				// dump($end_date.' end_up');
				$previous_txn_type = $customer_transaction->transaction_type;
				$previous_txn_date = $customer_transaction->deposit_date;
			}
	...
	protected function calculation($customer,$start_date,$end_date,$postingDate){
		...
		$cumilatePdate = CustomerDetail::whereCustomerId($customer->id)->wherePaymentType('accumulate')->first();

		// dump($start_date.' start_dateee');
		// dump($end_date.' end_dateee');
		...
		if($balance > 0){
			...
			foreach($customer_interest_percents as $key => $customer_interest_percent){
				...
				// echo "1";
				// dump($customer_interest_percent->count().'-->percent count');
				// dump($original_start_date.'-->original_start_date');
				// dump($interest_start_date.'-->interest_start_date');
				// dump($original_end_date.'-->original_end_date');
				// dump($customer_interest_percent->start_date.'-->customer_interest_percent');
				...
				if($payable_days > 0){
					...
					$customerrewards->posting_date = $cumilatePdate?date('Y-m-01',strtotime($postingDate)):$postingDate;
					$customerrewards->save();

                    $onePlusCumilateMonth = Carbon::parse($interest_start_date)->addMonths(1)->format('m');
					...
					$customerTransaction->remarks = 'Interest';
					$customerTransaction->deposit_date = $interest_end_date;
					$customerTransaction->posting_date = $cumilatePdate?date('Y-'.$onePlusCumilateMonth.'-01'):$postingDate;
	...
	protected function calculateAssociateCommissionPercent($start_date,$end_date,$associateId,$customer,$commission_rate,$balance,$postingDate){
		...
		if($payable_days > 0){
			...
			$associateTransaction->remarks = 'Commission';
    ...
    public function transactionHistory(Request $request)
    {
        ...
        $transactions = $transactions->orderByDesc('deposit_date')->paginate(50);
        // dd($transactions);
        return view('admin/customers/customer-transaction-history',compact('transactions'));
 ...
 //            })
	...
	public function updateInterestRateForm(Request $request){
		...
		return view('admin.customers.customer_edit_interest',compact('customer','cust_int_per','sum_comm'));
	}
	...
	public function updateInterestRate(Request $request)
	{
		// dump('real');
		// dd($request->all());
		$sum_of_cumm = $request->update_customer_interest+$request->sum_comm;
		// dd($sum_of_cumm);
		// if($sum_of_cumm > 36 || $sum_of_cumm < 36){
		// 	return redirect()->back()->with('error','Sum Of Commission is not Less Or Greater Than 36');
		// }
		$this->validate($request,[
        ...
        //Get max end date
        $max_end_date = CustomerInterestPercentage::where('customer_id',$request->customer_id)->select(['end_date'])->whereNotNull('end_date')->orderByDesc('end_date')->first();

        if($max_end_date && $max_end_date->end_date >= date("Y-m-d",strtotime($request->applicable_date))){
        	return redirect()->back()->with('error','You can not set applicable date less than '.$max_end_date->end_date);
        }

		$cust_interst->interest_percent = $request->update_customer_interest;
		$cust_interst->start_date = date("Y-m-d",strtotime($request->applicable_date));
	...
	public function temp_interest(Request $request)
	{
		// dump('temp');
		// dd($request->all());
	}
	...
	public function updateCommission(Request $request)
	{
		...
		// if(($totalassociatecommission+$request->update_associate_commission+$customer_interest_percent->interest_percent)>36 || ($totalassociatecommission+$request->update_associate_commission+$customer_interest_percent->interest_percent)<36){
		// 	return redirect()->back()->with('error','Sum of Interest and Commission can not be greater Or Less than 36!');
		// }
	...
	public function editInterestForm(Request $request)
	{
		// dd($request->all());
		$customer = User::where('id',$request->id)->where('login_type','customer')->firstOrFail();
		$sum_comm = $request->sumComm;
		return view('admin.customers.edit_interest',compact('customer','sum_comm'));
	}

	public function editInterest(Request $request)
	{
		// dd($request->all());
		// $sum_of_cumm = $request->sum_of_commission+$request->customer_interest;
		// if($sum_of_cumm > 36 ){	//|| $sum_of_cumm < 36
		// 	return redirect()->back()->with('error','Sum Of Commission is not Less Or Greater Than 36');
		// }
		...
		}else if($cust_interst->start_date >= date('Y-m-d',strtotime($request->applicable_date))){
			return redirect()->back()->with('error','Applicable date is not valid');
		}
		$cust_interst->active_status = 0;
		$cust_interst->end_date = Carbon::parse(date('Y-m-d',strtotime($request->applicable_date)))->subDays(1)->format('Y-m-d');
		$cust_interst->updated_at = date('Y-m-d');
		$cust_interst->save();

		$interst = new CustomerInterestPercentage;
		$interst->customer_id = $request->customer_id;
		$interst->start_date = date('Y-m-d',strtotime($request->applicable_date));
	...
	public function editCommission(Request $request)
	{
		...
		$totalassociatecommission = AssociateCommissionPercentage::where('associate_id','!=',$request->associate_id)->where('customer_id',$request->customer_id)->where('status',1)->sum('commission_percent');
		// if(($totalassociatecommission+$request->associate_withdrawl+$customer_interest_percent->interest_percent)>36 || ($totalassociatecommission+$request->associate_withdrawl+$customer_interest_percent->interest_percent)<36){
		// 	return redirect()->back()->with('error','Sum of Interest and Commission can not be greater Or Less than 36!');
		// }
		...
		$commission->start_date = date('Y-m-d',strtotime($request->start_date));
        ...
        $activity_log->save();
        associateHeirarchy($request->customer_id);
	...
	public function associateCommissionDestroy(Request $request){
		...
		if($user){
			...
			$customerInterest = AssociateCommissionPercentage::where('associate_id',$request->associate_id)->where('customer_id',$request->customer_id)->delete();//status = active_status//->where('id',$request->table_id)->where('status',1)
	...
	public function getCustomers(Request $request){
		// dd($request->all());
        if($request->has('term')){
            $results =  User::select('id','code','mobile',DB::raw('name as text'),DB::raw('CONCAT(name,"-", code) AS label'))->where('name','like','%'.$request->input('term').'%')->orWhere('code','like','%'.$request->input('term').'%')->where('login_type','customer')->where('lock_status',0)->get();
            return response()->json($results);
        }
    }
	...
	public function delete_gen_payouts($month,$year){
		...
		$activity_log = new ActivityLog;
		$activity_log->created_by = auth()->user()->id;
		$activity_log->statement = $month.'-'.$year.' - Payout has been deleted  Since '. date('d-m-Y');
		$activity_log->action_type = 'Delete Payout';
		$activity_log->save();
	...
	public function bulk_submit_transactions(Request $request)
	{
        // dd(date('Y-m-d',strtotime($request->deposit_date)));
		// dd($request->all());
		$this->validate($request,[
			'amount' => 'required',
			// 'remarks' => 'required',
			...
			if($request->transaction_type[$key] == 'deposit'){
				if($customerId != NULL){
					...
					if($cust_interest_percent){
						...
						$cust_investment->deposit_date = date('Y-m-d',strtotime($request->deposit_date));
					...
					$deposit->bank_id = $request->bank_id[$key];
					$deposit->cheque_dd_number = $request->cheque_dd_number[$key];
					$deposit->cheque_dd_date = $request->date[$key];
					$deposit->remarks = $request->remarks[$key];
                    ...
                    if($request->payment_type != 'cash'){
                        ...
                        $txnEntry->cheque_dd_date = isset($request->date[$key])?date('Y-m-d',strtotime($request->date[$key])):Null;
                        $txnEntry->respective_table_id = $deposit->id;
                        $txnEntry->respective_table_name = 'customer_transactions';
					...
					directAssociateCommission($deposit->customer_id);
				...
				if($customerId != NULL){
					...
					$deposit->bank_id = $request->bank_id[$key];
					$deposit->cheque_dd_number = $request->cheque_dd_number[$key];
                    $deposit->cheque_dd_date = isset($request->date[$key])?date('Y-m-d',strtotime($request->date[$key])):Null;
					$deposit->remarks = $request->remarks[$key];
					$deposit->status = 1;
					$deposit->created_by = \Auth::user()->id;
					$deposit->created_at = date('Y-m-d');
	...
	public function confirmPayout($month,$year)
	{
		...
		$activity_log = new ActivityLog;
		$activity_log->created_by = auth()->user()->id;
		$activity_log->statement = $month.'-'.$year.' - Payout has been generated  Since '. date('d-m-Y');
		$activity_log->action_type = 'Generate Payout';
		$activity_log->save();
 ...
 //        $transactions = $combine_transactions->orderBy('created_at')->paginate(50);
	...
	public function customer_delete_deposit_withdraw(Request $request)
	{
		...
		if($user){
			...
			directAssociateCommission($request->customer_id);
			// dd($cust_txn);
            $bankTxn = BankTransaction::where('respective_table_id',$request->table_id)->where('respective_table_name','customer_transactions')->first();
            if($bankTxn){
                $bankTxn->delete();
            }
    ...
    public function edit_bulktxn(Request $request)
	{
		...
		if($request->deposit_date != ''){
			...
			$toDate = date('Y-m-d',strtotime($request->deposit_date));
			// dump($fromDate);
			// dump($toDate);
			$customer_txn = CustomerTransactions::whereBetween('deposit_date', [$fromDate,$toDate])->where('transaction_type', '!=', 'interest')->get();
			// dd($customer_txn);
			$banks = CompanyBank::get();
	...
	{
		// dd($request->all());
		foreach($request->customer_id_old as $key => $customerId){
		// dd($request->deposit_date);
			$year = date('Y',strtotime($request->deposit_date));
	        $month = date('m',strtotime($request->deposit_date));
	        $entrylock = EntryLock::where('month',$month)->where('year',$year)->first();
	        if($entrylock != NULL && $entrylock->status == 0){
			...
			if($request->transaction_type[$key] == 'deposit'){
				if($request->customer_id[$key] != NULL){
					$cust_interest_percent = CustomerInterestPercentage::where('customer_id',$request->customer_id[$key])->first();
					if($cust_interest_percent){
                        $cust_investment = CustomerInvestment::where('customer_id', $customerId)->where('id', $request->respective_table_id[$key])->first();
                        // dd($cust_investment->customer_interest_rate);
                        if($cust_investment){
                            $cust_investment->customer_id = $request->customer_id[$key];
                            $cust_investment->amount = $request->amount[$key];
                            // $cust_investment->deposit_date = $request->deposit_date;
                            $cust_investment->customer_interest_rate = $cust_interest_percent->interest_percent?$cust_interest_percent->interest_percent:Null;
                            $cust_investment->created_by = \Auth::user()->id;
                            $cust_investment->updated_at = date('Y-m-d');
                            $cust_investment->save();
                            directAssociateCommission($customerId);
                        }
                    }
					$deposit = CustomerTransactions::where('customer_id', $customerId)->where('id',$request->table_id[$key])->where('transaction_type', 'deposit')->first();
					// dd($deposit);

					if($deposit != NULL){
                        $deposit->customer_id = $request->customer_id[$key];
                        $deposit->amount = $request->amount[$key];
                        $deposit->cr_dr = 'cr';
                        $deposit->payment_type = $request->payment_type[$key];
                        $deposit->transaction_type = 'deposit';
                        // $deposit->deposit_date = $request->deposit_date;
                        if($request->payment_type[$key] == 'cash'){
                            $deposit->bank_id = NULL;
                            $deposit->cheque_dd_number = NULL;
                            $deposit->cheque_dd_date = NULL;
                        }else{
                            $deposit->bank_id = $request->bank_id[$key];
                            $deposit->cheque_dd_number = $request->cheque_dd_number[$key];
                            $deposit->cheque_dd_date = $request->cheque_dd_date[$key];
                    ...
                    $bankTxn = BankTransaction::where('respective_table_id',$request->table_id[$key])->where('respective_table_name', 'customer_transactions')->where('bank_id', $request->bank_id_old[$key])->first();
                    // dd($bankTxn);
                    if($request->payment_type != 'cash' && $bankTxn != NULL){
                        ...
                        if($request->payment_type[$key] == 'cash'){
                            $bankTxn->bank_id = NULL;
                            $bankTxn->cheque_dd_number = NULL;
                            $bankTxn->cheque_dd_date = Null;
                        }else{
                            $bankTxn->bank_id = $request->bank_id[$key];
                            $bankTxn->cheque_dd_number = $request->cheque_dd_number[$key];
                            $bankTxn->cheque_dd_date = isset($request->date[$key])?date('Y-m-d',strtotime($request->date[$key])):Null;
                        }
                        $bankTxn->respective_table_id = $deposit->id;
                        $bankTxn->respective_table_name = 'customer_transactions';
				...
				if($customerId != NULL){
					$deposit = CustomerTransactions::where('customer_id', $customerId)->where('transaction_type', 'withdraw')->where('id', $request->table_id[$key])->first();
					if($request->customer_id[$key] != null){
						$deposit->customer_id = $request->customer_id[$key];
					}
					$deposit->amount = $request->amount[$key];
					...
					if($request->customer_id[$key] != null){
						directAssociateCommission($request->customer_id[$key]);
					}else{
						directAssociateCommission($deposit->customer_id);
					}
	...
	public function getCustomersForBulk(Request $request){
		...
		if($request->selectPayment == 'deposit'){
	        ...
	        if($request->has('term')){
	            $results =  User::select('id','status','code','mobile','lock_status',DB::raw('name as text'),DB::raw('CONCAT(name,"-", code) AS label'))->where('status',1)->where('lock_status',0)->where('login_type','customer')->where(function($q) use ($request){
	    	...
	    	// dd('2');
	        if($request->has('term')){
	            $results =  User::select('id','code','mobile',DB::raw('name as text'),DB::raw('CONCAT(name,"-", code) AS label'))->where('lock_status',0)->where('login_type','customer')->where(function($q) use ($request){
	            	return $q->where('name','like','%'.$request->input('term').'%')->orWhere('code','like','%'.$request->input('term').'%');
	            })->get();
	            return response()->json($results);
	        }
    ...
    public function delete_all_bulktxn(Request $request)
    {
    	...
    	$deposit_date = date('Y-m-d',strtotime($request->deposit_date));
    	$cust_txn = CustomerTransactions::where('transaction_type', '!=', 'interest')->whereBetween('deposit_date',[Carbon::parse($deposit_date)->format('Y-m-d'),Carbon::parse($deposit_date)->format('Y-m-d')])->get();
    	// dd($cust_txn);
		foreach($cust_txn as $txn){
			$cust_reward = CustomerReward::where('customer_id',$txn->customer_id)->where('reward_type','interest')->whereBetween('start_date',[date('Y-m-01',strtotime($request->deposit_date)),date('Y-m-t',strtotime($request->deposit_date))])->whereBetween('end_date',[date('Y-m-01',strtotime($request->deposit_date)),date('Y-m-t',strtotime($request->deposit_date))])->first();
            // dd($cust_reward);
			if(!$cust_reward){
		    	...
		    	$cust_txn = CustomerTransactions::where('customer_id',$txn->customer_id)->where('transaction_type', '!=', 'interest')->whereBetween('deposit_date',[Carbon::parse($deposit_date)->format('Y-m-d'),Carbon::parse($deposit_date)->format('Y-m-d')])->delete();
		    	// dd($cust_txn);

		    	$cust_invest = CustomerInvestment::where('customer_id',$txn->customer_id)->whereBetween('deposit_date',[Carbon::parse($deposit_date)->format('Y-m-d'),Carbon::parse($deposit_date)->format('Y-m-d')])->delete();
		    }
		}

    	return redirect()->back()->with('success', 'Delete All Transaction Which Deposit Or Withdrawl Date '.$deposit_date);
    }

	public function hold_customer_excel_export(Request $request){
        return Excel::download(new HoldCustomerReportExport($request),'hold_customer_excel_export.xlsx');
    }

	public function customer_pdc_issue_excel_export(Request $request){
        return Excel::download(new CustomerPDCIssueReportExport($request),'customer_pdc_issue_excel_export.xlsx');
    }

    public function associate_marked_in_customer(Request $request)
    {
        // dd($request->customer_id);
        $marked_associates = AssociateCommissionPercentage::where('customer_id',$request->customer_id)->groupBy('associate_id')->paginate(10);
        $customer = User::where('id',$request->customer_id)->where('login_type', 'customer')->first();
        return view('admin.customers.associate_markedin_customer',compact('marked_associates','customer'));
    }

    public function cust_lock_unlock($id)
    {
    	$customer = User::where('id',decrypt($id))->firstOrFail();
		$customer->lock_status = !$customer->lock_status;
		$customer->save();

		$activity_log = new ActivityLog;
        $activity_log->created_by = auth()->user()->id;
    	$activity_log->user_id = $customer->id;
        if($customer->lock_status == true){
        	$activity_log->statement = $customer->name.' ('.$customer->code.') is disable Since '.date('d-m-Y');
        }else{
        	$activity_log->statement = $customer->name.' ('.$customer->code.') is enable Since '.date('d-m-Y');
        }
        $activity_log->action_type = 'Change Lock Status';
        $activity_log->save();
		return redirect()->back()->with('success','Lock Status Updated Successfully!');
	}

	public function delCustomerPDC(Request $request)
	{
		$pdcCheque = CustomerSecurityCheque::where('id', $request->tableId)->where('customer_id', $request->custId)->first();

        return view('admin.customersecuritycheques.delPDC',compact('pdcCheque'));
	}

	public function destroyPDC(Request $request)
	{
        ...
        if($user){
            ...
            $activity_log->created_by = auth()->user()->id;
            $activity_log->user_id = $customer->id;
            $activity_log->statement = $customer->name.' ('.$customer->code.') Deleted PDC Since '.date('d-m-Y');
            $activity_log->action_type = 'Delete';
            $activity_log->save();

            CustomerSecurityCheque::where('id',$request->id)->where('customer_id',$request->customer_id)->delete();

            return back()->with('success','PDC deleted successfully.');
        }else{
            return redirect()->back()->with('error','PDC not deleted.');
        }
    }

    public function perCustomerLadgerBalance(Request $request)
    {
        if($request->input('from_date')!= '' && $request->input('to_date')!= ''){
            $transactions = CustomerTransactions::select('id',DB::raw('0 as associate_id'),'customer_id','amount','payment_type','cr_dr','status','cheque_dd_number','bank_id','cheque_dd_date','deposit_date','transaction_type','respective_table_id','respective_table_name','remarks','created_by','updated_by','created_at','updated_at')->where(function($q) use ($request){
            ...
            $transactions->whereIn('customer_id',function($q) use ($request){
                return $q->select('id')->where('code',auth()->user()->code)->from('users');
            });
