...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        // $tableName = 'tablename';
        // $whereIN = [];
        // $where = ['saac_user_id' => 1];
        // $stmt = 'select *'.$tableName;
        // if(!empty($whereIN)){
        //     $stmt .= ' where'.$where.' in '.$whereIN;
        // }elseif(!empty($where)){
        //     $stmt .= ' where'.$where;
        // }
        ...
        $self_associate_customer = AssociateCommissionPercentage::whereAssociateId(Auth::user()->id)->where('no_of_introducer',1)->where('status',1)->count();
        $total_associate_customer = AssociateCommissionPercentage::whereAssociateId(Auth::user()->id)->where('status',1)->count();
        ...
        // $top_associates = DirectAssociateCommission::select('associate_id','created_at',DB::raw('SUM(total_investment-total_withdraw) as top_invest'))->groupBy('associate_id')->orderByDesc('top_invest')->limit(5)
        // ->get();
        // dd($top_associates);
        $top_associates = AssociateCommissionPercentage::select(
                ...
                DB::raw(
                    ...
                    - SUM(COALESCE(CASE WHEN cr_dr = "dr" THEN amount ELSE 0 END, 0)) top_invest'))
            ->leftJoin('customer_transactions',
                function($join){
                    ...
                    ->groupBy(
                    'associate_commission_percentages.associate_id'
                )->orderByDesc('top_invest')->limit(5)
            ->get();
        // ->toSql();
        // dd($top_associates);
        ...
        while( date("Y-m",strtotime($start_date)) <= date("Y-m")){
            ...
            $associatedata['totcr'][$i] = AssociateTransactions::whereIn('customer_id',function($q){
                ...
                })->where('cr_dr','cr')->where('associate_id',auth()->user()->id)->whereBetween('deposit_date', [$start_date, $end_date])->sum('amount');

            $associatedata['totdr'][$i] = AssociateTransactions::whereIn('customer_id',function($q){
                return $q->select('customer_id')->from('associate_commission_percentages')->where('associate_id',auth()->user()->id)->where('status',1)->where('no_of_introducer',1)->where('deleted_at');
                })->where('cr_dr','dr')->where('associate_id',auth()->user()->id)->whereBetween('deposit_date', [$start_date, $end_date])->sum('amount');

            $totBusiness['totBal'][$i] = ($associatedata['totcr'][$i] - $associatedata['totdr'][$i]);

            // $associatedata['withdraw'][$i] = AssociateTransactions::where('associate_id',auth()->user()->id)->where('transaction_type','withdraw')->whereBetween('deposit_date', [$start_date, $end_date])->sum("amount");

            // $associatedata['commission'][$i] = AssociateTransactions::whereIn('customer_id',function($q){
            ...
            // })->whereBetween('deposit_date', [$start_date, $end_date])->groupBy('customer_id')->sum('amount');
       ...
       $totBusinesss = $totBusiness['totBal'];

        // $events = User::whereBetween('dob',[Carbon::now()->subDays(10)->format("Y-m-d"),Carbon::now()->format("Y-m-d")])->toSql();
        // dump(Carbon::now()->subDays(10)->format("Y-m-d"));
        // dump(Carbon::now()->format("Y-m-d"));
        // dd($events);

        return view('admin.dashboard',compact('total_customer','total_deposit','total_interest','total_associate_commission','top_associates','current_invest','top_invest','data','customerdata','associatedata','self_associate_customer','total_associate_customer','this_month_txn','this_month_comm','this_month_inter','last_month_comm','last_month_inter','total_associate','total_withdraw','last_month_deposit','last_month_withdraw','this_month_withdraw','totBusinesss'));
    }
