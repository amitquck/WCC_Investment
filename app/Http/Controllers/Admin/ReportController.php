...
class ReportController extends Controller
{
    ...
    */
    ...
    public function city_business(Request $request){
        ...
        if($request->city_id != null && $request->has('city_id') && $request->state_id != null && $request->has('state_id')){
            $users->whereHas('customerdetails',function($q) use ($request){
                return $q->where('city_id',$request->city_id)->where('state_id',$request->state_id);
            });
        }
        $users = $users->orderByDesc('created_at')->paginate(50);
