...
use App\Exports\DirectCustomerTransactionsReportExport;
use App\Exports\AssociateDirectCustomerReportExport;
use App\Exports\AssociateTxnHistReportExport;
use App\Exports\LastMonthAssBusiReportExport;
use App\Exports\ThisMonthAssBusiReportExport;
...
class AssociateController extends Controller
{
    ...
    {
        $associates = AssociateCommissionPercentage::whereAssociateId(Auth::user()->id)->where('no_of_introducer',1)->where('status',1)->paginate(20);
    ...
    public function perAssoLadgerBalance(Request $request)
    {
        $request->associate = auth()->user()->code;
        ...
        if($request->input('from_date')!= '' && $request->input('to_date')!= '' && $request->input('associate') != ''){
            ...
            $transactions->whereIn('associate_id',function($q) use ($request){
                return $q->select('id')->where('code',$request->associate)->from('users');
            });
    ...
    public function associate_transaction(Request $request)
    {
        // dd($request->all());
        $transactions = AssociateTransactions::select('id','associate_id','customer_id','amount','payment_type','cr_dr','status','cheque_dd_number','bank_id','cheque_dd_date','deposit_date','transaction_type','respective_table_id','respective_table_name','remarks','created_by','updated_by','created_at','updated_at')->where(function($q){
            return $q->where('transaction_type', '!=', 'commission')->where('associate_id',auth()->user()->id);
        });
        $transactions = $transactions->orderByDesc('deposit_date')
        ->paginate(50);
        // ->toSql();
        // dd($transactions);
        return view('admin/associates/associate_txn_hist',compact('transactions'));
    }

    public function heirarchy()
    {
        return view('admin.associates.associate_heirarchy');
    }

    public function direct_customer(Request $request)
    {
        // dd($request->all());
        $assId = decrypt($request->associate_id);
        if($assId){
            $assCust = AssociateCommissionPercentage::whereAssociateId($assId)->whereStatus(1)->paginate(50);
            if($assCust){
                return view('admin.associates.direct_customer',compact('assCust','assId'));
            }
    ...
    public function direct_customers(Request $request)
    {
        // dd($request->all());
        $assId = decrypt($request->associate_id);
        if($assId){
            $assCust = AssociateCommissionPercentage::whereAssociateId($assId)->whereStatus(1)->whereNoOfIntroducer(1)->paginate(50);
            if($assCust){
                return view('admin.associates.direct_customer',compact('assCust','assId'));
            }
            return back()->withError('Not Found');
    ...
    public function custTxn(Request $request)
    {
        // dd($request->all());
        $cust = User::where('id',$request->custId)->first();
        $customers = CustomerTransactions::where('customer_id',$request->custId)->where('transaction_type','!=','interest')->paginate(20);
        return view('admin/associates/associate_customer_txn',compact('customers','cust'));
    }

    public function export_associate_cust_txn(Request $request)
    {
        return Excel::download(new DirectCustomerTransactionsReportExport($request), 'direct_cust_txn.xlsx');
    }


    public function export_associate_direct_cust(Request $request)
    {
        return Excel::download(new AssociateDirectCustomerReportExport($request), 'direct_associate_cust.xlsx');
    }

    public function export_associate_txn_hist(Request $request)
    {
        return Excel::download(new AssociateTxnHistReportExport($request), 'associate_txn_hist.xlsx');
    }

    public function export_last_month_associate_business(Request $request)
    {
        return Excel::download(new LastMonthAssBusiReportExport($request), 'last_month_associate_business.xlsx');
    }

    public function export_this_month_associate_business(Request $request)
    {
        return Excel::download(new ThisMonthAssBusiReportExport($request), 'This_month_associate_business.xlsx');
    }

    public function totCustBusiLastMon()
    {
        ...
        $customers = $customers->groupBy('customer_id')->paginate(50);
        return view('admin/associates/cust_busi_last_month',compact('customers'));
    }

    public function selfCustBusiLastMon()
    {
        $customer = AssociateTransactions::whereIn('customer_id',function($q){
    ...
    public function totCustBusiThisMon()
    {
        $customer = AssociateTransactions::whereIn('customer_id',function($q){
            return $q->select('customer_id')->from('associate_commission_percentages')->where('associate_id',auth()->user()->id)->where('status',1)->where('deleted_at');
        })->whereBetween('deposit_date',[date('Y-m-01'),date('Y-m-t')]);

        $associate = AssociateTransactions::where('associate_id',auth()->user()->id)->where('transaction_type','withdraw')->whereBetween('deposit_date',[date('Y-m-01'),date('Y-m-t')]);

        $customers = $customer->union($associate);
        $customers = $customers->groupBy('customer_id')->paginate(50);
        return view('admin/associates/cust_busi_this_month',compact('customers'));
    }

    public function selfCustBusiThisMon()
    {
        $customer = AssociateTransactions::whereIn('customer_id',function($q){
            return $q->select('customer_id')->from('associate_commission_percentages')->where('associate_id',auth()->user()->id)->where('status',1)->where('no_of_introducer',1)->where('deleted_at');
        })->whereBetween('deposit_date',[date('Y-m-01'),date('Y-m-t')]);
