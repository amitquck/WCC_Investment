	      	...
	      	if(\Auth::user()->login_type == 'superadmin' || \Auth::user()->login_type == 'employee'){
		        ...
		        if($request->customer_name != null && $request->has('customer_name') ){
		            ...
		            $users->whereHas('customertransactions',function($q) use ($request){
		                return $q->where('amount',$request->customer_deposit)->where('transaction_type','deposit');
		...
		public function store(Request $request){
			...
			try{
				...
				$customer->customerdetails()->create([
					...
					'nominee_dob' => isset($request->nominee_dob)?date('Y-m-d',strtotime($request->nominee_dob)):NULL,
			...
			if($request->isMethod('post')){
				...
				if($customer->count() > 0){
					...
					$customer->customerdetails()->update([
						'image' => $name,
						'dob' => date('Y-m-d',strtotime($request->dob))?date('Y-m-d',strtotime($request->dob)):'',
						'age' => $request->age,
						'nationality' => $request->nationality,
						'sex' => $request->sex,
						'father_husband_wife' => $request->father_husband_wife,
		...
		if($balance > 0){
			...
			foreach($customer_interest_percents as $key => $customer_interest_percent){
				...
				if($payable_days > 0){
					...
					$customerTransaction->posting_date = $cumilatePdate?date('Y-m-01',strtotime($postingDate)):$postingDate;
