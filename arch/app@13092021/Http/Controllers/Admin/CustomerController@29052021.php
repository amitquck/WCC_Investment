				...
				if($customer->count() > 0){
					...
					if($request->cheque_issue_dates || $request->cheque_issue_date){
						foreach($request->ids as $key => $id){
					        ...
					        if($per_customer){
								$per_customer->cheque_issue_date = date('Y-m-d',strtotime($request->cheque_issue_date[$key]));
								$per_customer->cheque_maturity_date = date('Y-m-d',strtotime($request->cheque_maturity_date[$key]));
								...
								$per_customer->cheque_date = date('Y-m-d',strtotime($request->cheque_date[$key]));
					...
					if($request->cheque_issue_dates){
						foreach($request->cheque_issue_dates as $key => $issue_date){
							...
							$customer->customersecuritycheque()->update([
								'cheque_issue_date' => date('Y-m-d',strtotime($issue_date)),
								'cheque_maturity_date' => date('Y-m-d',strtotime($request->cheque_maturity_dates[$key])),
								...
								'cheque_date' => date('Y-m-d',strtotime($request->cheque_dates[$key])),
								'scan_copy' => $customerscan,
								'status' => 1,
...
// dump($balance);
 ...
 //        $transactions = $combine_transactions->orderBy('created_at')->paginate(50);
    ...
    public function delete_all_bulktxn(Request $request)
    {
    	...
    	$cust_txn = CustomerTransactions::where('transaction_type', '!=', 'interest')->whereBetween('deposit_date',[Carbon::parse($deposit_date)->format('Y-m-d'),Carbon::parse($deposit_date)->format('Y-m-d')])->delete();
    	// dd($cust_txn);

    	$cust_invest = CustomerInvestment::whereBetween('deposit_date',[Carbon::parse($deposit_date)->format('Y-m-d'),Carbon::parse($deposit_date)->format('Y-m-d')])->delete();
