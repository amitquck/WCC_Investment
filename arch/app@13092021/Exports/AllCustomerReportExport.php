...
class AllCustomerReportExport implements FromCollection
{
    ...
    */
    public function collection()
    {
        ...
        }else if($this->customer_deposit != null && $this->has('customer_deposit')){
            $users->whereHas('customertransactions',function($q){
                return $q->where('amount',$this->customer_deposit)->where('transaction_type','deposit');
            });
        }
		...
		foreach($db_records as $key => $record){
	        ...
	        $records[$key]['Enable/Disable Status'] = $record->lock_status?'Enable':'Disable';
