...
class AssociateTransactions extends Model
{
    ...
    {
        return AssociateReward::where('associate_id',$this->associate_id)->where('reward_type','commission')->where('month', '>=', Carbon::parse($this->deposit_date)->format('m'))->where('year', '>=',Carbon::parse($this->deposit_date)->format('Y'))->toSql();//
    }
}
