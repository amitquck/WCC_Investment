...
class ReportController extends Controller
{
    ...
    public function monthly_payout(Request $request)
    {
        ...
        // dd('hk');
            if($request->payment_type == '0'){
                ...
                })->where('month',$month)->where('year',$request->year)->groupBy(['customer_id','month','year'])->orderByDesc('deposit_date')->paginate(50);
            }else{
                $cust_reward = CustomerReward::whereIn('customer_id',function($q) use ($request){
            ...
            $cust_reward = CustomerReward::where('month',$month)->where('year',$request->year)->groupBy(['customer_id','month','year'])->orderByDesc('deposit_date')->paginate(50);
        }else{
    ...
    {
        return Excel::download(new DebitorCreditorReportExport($request),'debitor_creditor_list.xlsx');
    }
