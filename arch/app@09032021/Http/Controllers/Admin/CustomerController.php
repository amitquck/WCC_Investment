<?php 
	...
	public function payoutGenerates(Request $request)
	{
		...
		// return redirect()->route('admin.confirmation_generated_payouts');
		// return view('admin.customers.confirmation_generated_payouts');
...
// dd($interest);
 ...
 // dd($l_date);
		...
		// 	$reward->delete();
		// });
		$cust_reward_temps = CustomerRewardTemp::where('month',$month)->where('year',$year)->get();
		...
		$cust_txn_temps = CustomerTransactionTemp::whereBetween('deposit_date', [$f_date, $l_date])->get();
		// dd($cust_txn_temps);
		foreach($cust_txn_temps as $cust_txn_temp){
			...
			$cust_txn->respective_table_id = $cust_txn_temp->respective_table_id;
		...
		$cust_reward_temps = CustomerTransactionTemp::whereBetween('deposit_date', [$f_date, $l_date])->forceDelete();

		$asso_reward_temps = AssociateRewardTemp::where('month',$month)->where('year',$year)->get();
		...
		$cust_reward_temps = AssociateRewardTemp::where('month',$month)->where('year',$year)->forceDelete();

		$asso_txn_temps = AssociateTransactionTemp::whereBetween('deposit_date', [$f_date, $l_date])->get();
		...
		$cust_reward_temps = AssociateTransactionTemp::whereBetween('deposit_date', [$f_date, $l_date])->forceDelete();
	...
	{	
		// SELECT * FROM customer_investments WHERE NOT EXISTS (SELECT respective_table_id FROM customer_transactions WHERE customer_transactions.respective_table_id = customer_investments.id and customer_transactions.respective_table_name = 'customer_investments') 
	// $missing_data = CustomerTransactions::select('customer_transactions.respective_table_id','customer_investments.*')
	// ->join('customer_investments', function($join){
	// 	$join->on('customer_investments.id' ,'!=', 'customer_transactions.respective_table_id');
	// })->toSql();
		...
		foreach($request->customer_id as $key => $customerId){
			if($request->transaction_type[$key] == 'deposit'){
				if($customerId != NULL){
					...
					$cust_interest_percent = CustomerInterestPercentage::where('customer_id',$customerId)->first();
					// dump($cust_interest_percent);
					$cust_investment = CustomerInvestment::where('customer_id', $customerId)->first();
					if($cust_investment){
						// $cust_investment->customer_id = $customerId;
						$cust_investment->amount = $request->amount[$key];
						// $cust_investment->deposit_date = $request->deposit_date;
