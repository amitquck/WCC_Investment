...
use App\Exports\AllEmployeeReportExport;
...
class EmployeeController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $activity_log->user_id = $employee->id;     
        $activity_log->statement = $request->name.' ('.$request->employee_id.') Added Since '. date('d-m-Y');  
        $activity_log->action_type = 'Add Employee';
        $activity_log->save();
    ...
    public function employeeUpdate(Request $request){
        ...
        $id = $request->id;
        $this->validate($request,[
            'employee_id' => 'required|max:20|unique:users,code,'.$id.',id,deleted_at,NULL',
            'name' => 'required',
            'mobile' => 'required|numeric|min:10',
            'email' => 'required|email',
        ...
        $activity_log->created_by = auth()->user()->id;
        $activity_log->user_id = $employee->id;     
        $activity_log->statement = $employee->name.' ('.$employee->code.') Updated Since '. date('d-m-Y');  
        $activity_log->action_type = 'Update Employee';
        $activity_log->save();
    ...
    public function status($id){
        // dd($id);
            ...
            $activity_log->created_by = auth()->user()->id; 
            $activity_log->user_id = $employee->id;      
            $activity_log->statement = $employee->name.' ('.$employee->code.') Change Status Since '. date('d-m-Y');  
		...
		public function destroy($id){
            ...
            $employee = User::where('id',decrypt($id))->first();
            $activity_log = new ActivityLog;
            $activity_log->created_by = auth()->user()->id;
            $activity_log->user_id = $employee->id;       
            $activity_log->statement = $employee->name.' ('.$employee->code.') Deleted Since '. date('d-m-Y');  
            $activity_log->action_type = 'Employee Delete';
            $activity_log->save();
...
// dd();
    ...
    public function employee_excel_export(Request $request)
    {
        return Excel::download(new AllEmployeeReportExport($request),'employee_excel_export.xlsx');
    }
