...
class ThisMonthBusinessCategoryReportExport implements FromCollection
{
    ...
    */
    public function collection()
    {
        ...
        if($this->q != '' && $this->q == 'customer'){
            ...
            $associates = AssociateCommissionPercentage::select(
                ...
                function($join){
                    ...
                    ->whereBetween('deposit_date',[$this->from_date,$this->to_date])
                    ->groupBy(
                    'associate_commission_percentages.associate_id'
            ...
            foreach($db_records as $key => $record){
                if($record->cust_balance != 0 && $record->getState != ''){
				    $key++;
            ...
            $records[] = ['State','City','Business Amount'];
            foreach($db_records as $key => $record){
                if($record->getCity != '' && $record->cust_balance != 0){
    				$key++;
                    $records[$key]['State'] = getStateName($record->city_id);
        ...
        $activity_log->statement = 'This Month Business Category Report Excel Export By '.auth()->user()->name.' Since At '.date('d-m-Y');
