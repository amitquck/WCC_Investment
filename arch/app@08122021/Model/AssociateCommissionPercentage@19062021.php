...
class AssociateCommissionPercentage extends Model
{
    ...
    public function associateTotalBusiness(){
        ...
        foreach($customers as $key => $customer){
            ...
            // $cust_txn = CustomerTransactions::select('*',DB::raw('SUM((amount) * ( CASE WHEN (SELECT sum(amount) FROM customer_transactions where customer_id = "'.$customer->customer_id.'" and cr_dr = "cr") THEN 1 WHEN (SELECT sum(amount) FROM customer_transactions where customer_id = "'.$customer->customer_id.'" and cr_dr = "dr") THEN -1 END )) AS cust_balance'))->groupBy('customer_id')->orderByDesc('cust_balance')
            // // ->get();
            // ->toSql();
            // dd($cust_txn);
        }
