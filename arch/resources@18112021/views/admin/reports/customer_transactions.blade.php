...
@section('content')
  ...
  @if($transactions)
    ...
    @endphp
    @if(empcan('export_customer_ladger_balance') || Auth::user()->login_type == 'superadmin')
    ...
    <form action="" method="get">
        <div class="col-md-12">
            ...
            <div class="col-md-3">
                ...
                <input type="text" name="name" class="form-control" placeholder="Code" id="cust_invest"><input type="hidden" id="hidden_cust_invest" name="customer">
            </div>
  ...
  <div class="card card-style-1 mt-3">
    @if (auth()->user()->login_type == 'customer') <span class="bg-primary text-center text-white">{{ ucwords(auth()->user()->name) }} ({{ auth()->user()->code }})</span> @endif
              ...
              @foreach($transactions as $key => $transaction)
                ...
                <tr>
                  <td>{{($transactions->currentpage()-1)*$transactions->perpage()+$key+1}}.</td>
                    @if (auth()->user()->login_type != 'customer')
                        <td class="text-info"><a @if(empcan('view_detail_customer_ladger_balance') || Auth::user()->login_type == 'superadmin') href="{{route('admin.customer_detail',encrypt($transaction->customers->id))}}" @endif data-toggle="tooltip" title="View Details">{{($transaction->customers->code)}}</td>
...
<script type="text/javascript">
  $('#cust_invest').autocomplete({
    ...
    select:function(event,ui){
      ...
      $('#hidden_cust_invest').val(ui.item.code);
    }
  });
