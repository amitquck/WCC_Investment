    ...
    <div class="row">
     ...
     <p>OR</p> -->
      <div class="col-md-3">
        ...
        <input type="text" name="name" class="form-control" placeholder="Code Or Name" id="asso_invest"><input type="hidden" name="associate" id="hidden_asso_invest">
      </div>
      ...
      <table class="table table-nostretch table-align-middle mb-0">
        ...
        <tbody>
            ...
            @php $count = 0; @endphp
              <tr>
                <td>{{$count+1}}. </td>
                <td><a href="{{route('admin.associate_view',$associate->id)}}" class="text-info" data-toggle="tooltip" title="View Detail">{{$associate->code}}</a></td>
                <td>{{$associate->name}}</td>
                <td>
                  @if($associate->associatebalance() > 0)
                    <p><a @if(empcan('view_detail_associate_ladger_balance') || Auth::user()->login_type == 'superadmin') href="{{route('admin.list_associate_ladger',['associate'=>$associate->code])}}" @endif class="text-success" data-toggle="tooltip" title="View Detail">₹ {{$associate->associatebalance()}}</a></p>
                  @else
                    <p><a href="{{route('admin.list_associate_ladger',['associate'=>$associate->code])}}" class="text-danger" data-toggle="tooltip" title="View Detail">₹ {{$associate->associatebalance()}}</a></p>
                  @endif
                </td>
...
<script type="text/javascript">
  $('#asso_invest').autocomplete({
    ...
    select:function(event,ui){
      ...
      $('#hidden_asso_invest').val(ui.item.code);
    }
  });
