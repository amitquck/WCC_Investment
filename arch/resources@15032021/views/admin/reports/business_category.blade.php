            ...
            @if($cust_txn->count()>0)
              @foreach($cust_txn as $key => $txn)
                <tr>
                  ...
                  <td class="text-success">₹ {{$txn->customer_wise_business}}</td>
                </tr>
              @endforeach
            @else
            ...
            @if($associates->count()>0)
              @foreach($associates as $key => $associate)
                  <tr>
                    ...
                    <td class="text-success">₹ {{$associate->associate_wise_business}}</td>
                  </tr>
              @endforeach
            @else
              <tr><td colspan="8"><h4 class="text-center text-danger">No Record Found</h4></td></tr>
            ...
            @if($states->count()>0)
              @foreach($states as $key => $state)
                  <tr>
                    ...
                    <td class="text-success">₹ {{$state->state_wise_business}}</td>
                  </tr>
              @endforeach
            @else
              <tr><td colspan="8"><h4 class="text-center text-danger">No Record Found</h4></td></tr>
            ...
            @if($cities->count()>0)
              @foreach($cities as $key => $city)
                  <tr>
                    ...
                    <td class="text-success">₹ {{$city->city_wise_business}}</td>
                  </tr>
              @endforeach
            @else
              <tr><td colspan="8"><h4 class="text-center text-danger">No Record Found</h4></td></tr>
