            ...
            @if($transactions->count()>0)
              @foreach($transactions as $key => $transaction)
                <tr>
                  ...
                  <td>
                    @php $deposit_date = Carbon\Carbon::parse($transaction->deposit_date)->format('j-M-Y'); @endphp
                    {{$deposit_date}}
                  </td>
                  <td>
                  ...
                  <td>
                    @if(Auth::user()->login_type == 'superadmin' && $transaction->transaction_type == 'deposit')
                      <a class="btn btn-xs text-primary @if(!$transaction->is_month_payout_generate) edit_deposit @else disabled @endif" data-toggle="tooltip" title="Edit Investment" data-id="{{$transaction->customer_id}},{{$transaction->id}}"><i class="material-icons">edit</i></a>
                      <a class="btn btn-link btn-icon bigger-130 text-danger @if(!$transaction->is_month_payout_generate) delete_confirm @else disabled @endif" data-toggle="tooltip" title="Delete" data-id="{{$transaction->customer_id}},{{$transaction->id}}"><i class="material-icons">delete_outline</i></a>
                    @elseif(Auth::user()->login_type == 'superadmin' && $transaction->transaction_type == 'withdraw')
                      <a class="btn btn-xs text-success @if(!$transaction->is_month_payout_generate) edit_withdraw @else disabled @endif" data-toggle="tooltip" title="Edit Withdraw" data-id="{{$transaction->customer_id}},{{$transaction->id}}"><i class="material-icons">edit</i></a>
                      <a class="btn btn-link btn-icon bigger-130 text-danger @if(!$transaction->is_month_payout_generate) delete_confirm @else disabled @endif" data-toggle="tooltip" title="Delete" data-id="{{$transaction->customer_id}},{{$transaction->id}}"><i class="material-icons">delete_outline</i></a>
                    @endif
                  </td>
