              ...
              <td class="table-actions">
                <div class="dropdown">
                    ...
                    @if(empcan('edit_commission_customer') || Auth::user()->login_type == 'superadmin')
                      <a @if(!$user->lock_status) href="{{route('admin.customer_associate_commission',encrypt($user->id))}}" class="btn btn-default btn-link btn-icon bigger-130 text-info" data-toggle="tooltip" title="Edit Commission"  data-id="{{$user->id}}" @else href="javascript:void(0)" class="btn btn-default btn-link btn-icon bigger-130 text-info" data-toggle="tooltip" title="Edit Commission"  data-id="{{$user->id}}" @endif ><i class="material-icons">edit</i></a>
                    @endif

                    @if(empcan('add_withdraw_customer') || Auth::user()->login_type == 'superadmin')
                      <a class="btn  btn-default bigger-130 text-danger @if($user->customerdetails->payment_type == 'hold' || $user->customer_current_balance() <= 0 || $user->lock_status != 0) disabled @elseif($user->hold_status) customerWithdrawForm @else disabled @endif " data-toggle="tooltip" title="Add Withdraw"  data-id="{{$user->id}}"><i class="material-icons">account_balance_wallet</i></a>   
                    @endif   
