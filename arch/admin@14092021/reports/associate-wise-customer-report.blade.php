        ...
        <div class="col-md-3">
          <tr class="column-filter" id="filter">
            <th>
              ...
              <input type="text" class="form-control" placeholder="Associate Search" name="" id="cust_invest" autocomplete="off"  data-toggle="tooltip" title="Associate Search"><input type="hidden" name="id" id="hidden_cust_invest">
            </th>
          </tr>
        </div>
        <div class="col-md-1">
          ...
          @if($associate_txn)
            @foreach($associate_txn as $key => $investment)
              <tr>
                ...
                @else
                  <td>
                    @if($investment->customer->customer_current_balance() > 0)
                      <p class="text-success">₹ {{$investment->customer->customer_current_balance()}}</p>
                    @else
                      <p class="text-danger">₹ {{$investment->customer->customer_current_balance()}}</p>
                    @endif
                  </td>
                @endif
              </tr>
