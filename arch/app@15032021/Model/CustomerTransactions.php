...
class CustomerTransactions extends Model
{
    ...
    {
        $txn = CustomerTransactions::where('customer_id',$this->customer_id)->where('cr_dr','cr')->where('transaction_type','deposit')->whereBetween('deposit_date',[date('Y-m-01'),date('Y-m-t')])->sum('amount');
    ...
    {
        $txn = CustomerTransactions::where('customer_id',$this->customer_id)->where('cr_dr','cr')->where('transaction_type','deposit')->whereBetween('deposit_date',[date('Y-m-01',strtotime('-1 months')),date('Y-m-t',strtotime('-1 months'))])->sum('amount');
