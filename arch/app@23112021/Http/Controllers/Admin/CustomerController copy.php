				...
				if($customer->count() > 0){
					...
					if($request->cheque_issue_dates || $request->cheque_issue_date){
						...
						foreach($request->ids as $key => $id){
							...
							// $customerscan = $s_cheque->scan_copy;
							// dd($customerscan);
						 //   }
					        if($request->has('scan_copy.'.$key)){
						    $customerscan='';
 ...
 //        $transactions = $combine_transactions->orderBy('created_at')->paginate(50);
		...
		// dd($customerId);
			...
			if($request->transaction_type[$key] == 'deposit'){
				if($request->customer_id[$key] != NULL){
                    ...
                    $deposit = BankTransaction::where('respective_table_id',$request->table_id[$key])->where('respective_table_name', 'customer_transactions')->first();
    ...
    public function delete_all_bulktxn(Request $request)
    {
		...
		foreach($cust_txn as $txn){
			$cust_reward = CustomerReward::where('customer_id',$txn->customer_id)->where('reward_type','interest')->first();
			if(!$cust_reward){
