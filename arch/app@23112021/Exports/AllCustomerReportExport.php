...
class AllCustomerReportExport implements FromCollection
{
	...
	public function __construct($request){
		...
		$this->customer_mobile = $request->customer_mobile;
		$this->customer_bank_account = $request->customer_bank_account;
		$this->customer_deposit = $request->customer_deposit;
    ...
    */
    ...
    {
    	$records = [];
    	$records[] = ['Code','Name','Father Name','Mobile','Email','Gender','DOB','Age','Nationality','Address One','Address Two','Zipcode','City','State','Country','Account Holder Name','Bank Name','Account No','IFSC Code','Pan Number','Payment Type','Nominee Name','Nominee Age','Nominee DOB','Nominee Gender','Relationship With Applicant','Nominee Address One','Nominee Address Two','Nominee Zipcode','Nominee City','Nominee State','Nominee Country','Status','Enable/Disable Status','Member Since'];
    	$users = User::whereLoginType('customer');

    	if($this->customer_name != null){
            $users->where('name','like','%'.$this->customer_name.'%');
        }else if($this->customer_mobile != null){
            $users->where('mobile','like','%'.$this->customer_mobile.'%');
        }else if($this->customer_id != null){
            $users->where('code','like','%'.$this->customer_id.'%');
        }else if($this->customer_bank_account != null){
            $users->whereHas('customerdetails',function($q){
                return $q->where('account_number','like','%'.$this->customer_bank_account.'%');
            });
        }else if($this->state_id != null && $this->city_id == 'Select City'){
	        $users->whereHas('customerdetails',function($q){
	            return $q->where('state_id',$this->state_id);
	        });
        }else if($this->state_id != null && $this->city_id != null){
            $users->whereHas('customerdetails',function($q){
                return $q->where('city_id',$this->city_id);
            });
        }else if($this->customer_deposit != null ){
            $users->whereHas('customertransactions',function($q){
                  return $q->select('*',DB::raw('SUM((amount) * ( CASE WHEN cr_dr = "cr" THEN 1 WHEN transaction_type = "withdraw" THEN -1 END )) AS cust_balance'))->groupBy('customer_id')->having('cust_balance',$this->customer_deposit);
            });
        }
        
		$db_records = $users->orderByDesc('created_at')->get();
		foreach($db_records as $key => $record){
			// dd($record->customerdetails->country->name);
			$key++;
	        ...
	        $records[$key]['DOB'] = (isset($record->customerdetails->dob) && $record->customerdetails->dob != '1970-01-01')?$record->customerdetails->dob:'N/A';
	        ...
	        $records[$key]['City'] = isset($record->customerdetails->citycity->name)?$record->customerdetails->citycity->name:'N/A';
	        $records[$key]['State'] = isset($record->customerdetails->statestate->name)?$record->customerdetails->statestate->name:'N/A';
	        $records[$key]['Country'] = isset($record->customerdetails->countrycountry->name)?$record->customerdetails->countrycountry->name:'N/A';
	        ...
	        $records[$key]['Bank Name'] = isset($record->customerdetails->bank_name)?$record->customerdetails->bank_name:'N/A';
	        ...
	        $records[$key]['Nominee City'] = isset($record->customerdetails->city->name)?$record->customerdetails->city->name:'N/A';
	        $records[$key]['Nominee State'] = isset($record->customerdetails->state->name)?$record->customerdetails->state->name:'N/A';
	        $records[$key]['Nominee Country'] = isset($record->customerdetails->country->name)?$record->customerdetails->country->name:'N/A';
	        $records[$key]['Staus'] = $record->status?'Active':'Inactive';
	        $records[$key]['Enable/Disable Status'] = $record->lock_status?'Disable':'Enable';
