              ...
              <tr>
                ...
                @if($customer->login_type == 'customer')
                  ...
                  @php $to_date = date($year.'-'.$month.'-'.'31'); @endphp
                    <td class="text-danger"><a href="{{route('admin.customer_transactions',['customer'=>$customer->code, 'from_date' => $from_date, 'to_date' => $to_date])}}" class="text-danger" data-toggle="tooltip" title="View Ladger Balance">₹ {{$customer->customer_monthly_balance_recalculate($month,$year)}}</a></td>
                  @else
