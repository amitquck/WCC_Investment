...
<form action="" method="get">
  <div class="col-md-12">
    <div class="row">
      ...
      <div class="col-md-3">
        <label for="customer">Customer <sup class="text-danger">*</sup></label>
        ...
        <tbody>
            ...
            @php $total_credit = $total_debit = $running_balance = 0; @endphp
              @foreach($transactions as $key => $transaction)
                ...
                @if($transaction->customer_current_balance($transaction->customer_id,Request::input('from_date'),Request::input('to_date')) < 0)
                  <td class="text-danger">₹ {{$transaction->customer_current_balance($transaction->customer_id,Request::input('from_date'),Request::input('to_date'))}}</td>
                  <td class="text-success">₹ 0.00</td>
                @else
                  <td class="text-danger">₹ 0.00</td>
                  <td class="text-success">₹ {{$transaction->customer_current_balance($transaction->customer_id,Request::input('from_date'),Request::input('to_date'))}}</td>
                @endif 
