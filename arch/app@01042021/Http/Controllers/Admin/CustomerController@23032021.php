...
// dd('bvtgksd');
		...
		if($balance > 0){
			...
			// dump($associate_commission_percents->count().' count1');
			if(!$associate_commission_percents->count()){
				...
				->get();
				// ->toSql();
				// dd($associate_commission_percents);
				
				// if(!$associate_commission_percents->count()){
				//  	$associate_commission_percents = AssociateCommissionPercentage::where('status',1)->where('customer_id',$customer->id)->get();//status = active_status
				// }
			}
			// dump($associate_commission_percents->count().' count2');
			foreach($associate_commission_percents as $key => $associate_commission_percent){
				dump($associate_commission_percent);
				$associate_first_record = $associate_commission_percent->where('customer_id',$customer->id)->where('associate_id',$associate_commission_percent->associate_id)->orderBy('start_date')->first();
				$swap_dates = false;

				if($original_start_date > $associate_commission_percent->start_date){
					dump('1');
					$start_date = $original_start_date;
				}else if($associate_first_record->id == $associate_commission_percent->id){
					dump('4');
					$start_date = $associate_first_record->start_date;
				}
