            ...
            @if($cust_txn->count()>0)
              @foreach($cust_txn as $key => $txn)
                <tr>
                  ...
                  <td>
                    @if($txn->customer_wise_business > 0)
                      <a href="{{route('admin.transactions',['customer'=>$txn->customers->code])}}" data-toggle="tooltip" title="View Ladger Balance" class="text-success">₹ {{$txn->customer_wise_business}}</a>
                    @else
                      <a href="{{route('admin.transactions',['customer'=>$txn->customers->code])}}" data-toggle="tooltip" title="View Ladger Balance" class="text-danger">₹ {{$txn->customer_wise_business}}</a>
                    @endif
                  </td>
                </tr>
              @endforeach
            ...
            @if($associates->count()>0)
              @foreach($associates as $key => $associate)
                  <tr>
                    ...
                    <td>
                      @if($associate->associateTotalBusiness() > 0)
                        <a href="{{route('admin.associate_business_report',['id'=>$associate->associate->id])}}" data-toggle="tooltip" title="View Ladger Balance" class="text-success"> ₹ {{$associate->associateTotalBusiness()}}</a>
                      @else
                        <a href="{{route('admin.associate_business_report',['id'=>$associate->associate->id])}}" data-toggle="tooltip" title="View Ladger Balance" class="text-danger"> ₹ {{$associate->associateTotalBusiness()}}</a>
                      @endif
                    </td>
                  </tr>
              @endforeach
            ...
            @if($states->count()>0)
              @foreach($states as $key => $state)
                  <tr>
                    ...
                    <td>
                      @if($state->state_wise_business > 0)
                        <a href="{{route('admin.state_business',['state_id'=>$state->id])}}" data-toggle="tooltip" title="View Ladger Balance" class="text-success"> ₹ {{$state->state_wise_business}}</a>
                      @else
                        <a href="{{route('admin.state_business',['state_id'=>$state->id])}}" data-toggle="tooltip" title="View Ladger Balance" class="text-danger"> ₹ {{$state->state_wise_business}}</a>
                      @endif
                    </td>
                  </tr>
              @endforeach
            ...
            @if($cities->count()>0)
              @foreach($cities as $key => $city)
                  <tr>
                    ...
                    <td>
                      @if($city->city_wise_business > 0)
                        <p class="text-success"> ₹ {{$city->city_wise_business}}</p>
                      @else
                        <p class="text-danger"> ₹ {{$city->city_wise_business}}</p>
                      @endif
                    </td>
                  </tr>
              @endforeach
