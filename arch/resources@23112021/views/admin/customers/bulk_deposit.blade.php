...
<ol class="breadcrumb breadcrumb-style2">
    ...
    <li class="breadcrumb-item active" aria-current="page">Customer Add Bulk Transaction</li>
</ol>
</nav>
<!-- /Breadcrumb -->
@include('flash')	
            ...
            <thead>
              <tr>
                <th>
                    ...
                    <input type="text" name="deposit_date" class="form-control datepicker" value="{{date('d-m-Y')}}">
                </th>
                <th>
...
$('.datepicker').datepicker({
 ...
 $(document).ready(function() {
   ...
   $(wrapper).on("change",".select_payment", function(e){
      $(this).parents('tr:first').find('.cust_search').val('').next().val('');
   });
...
function autocomplete_customer(){
    $('.cust_search').autocomplete({
        ...
        // alert(selectPayment);
          $.getJSON("{{route('admin.getCustomersForBulk')}}", { term: this.element.val() ,selectPayment:this.element.parents('tr:first').find('.select_payment').val()}, 
                    response);
        },
        minLength: 2,
