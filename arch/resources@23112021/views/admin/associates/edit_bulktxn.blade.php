  ...
  <div class="col-md-12">
    <div class="row">
        <div class="col-md-3">
          ...
          <input type="text" name="deposit_date" class="form-control datepicker" placeholder="dd-mm-yyyy" autocomplete="off">
        </div>
        <div class="col-md-1">
            ...
            @foreach($associate_txn as $key => $txn)
              <tr>
                ...
                <th>
                  ...
                  <select class="form-control" name="transaction_type[]" id="select_payment" @if(!empty($txn->is_month_payout_generate))  @else disabled="disabled" @endif>
                      <option value="">Select Type</option>
                      <option value="withdraw" @if($txn->transaction_type == 'withdraw') selected="selected" @endif>Withdrawl</option>
                  </select>
                </th>
                <td>
                  <input type="text" name="" class="form-control asso_search" placeholder="Customer" id="asso_search" autocomplete="off" value="{{$txn->associate->name.' ('.$txn->associate->code.')'}}" @if(!empty($txn->is_month_payout_generate))  @else disabled="disabled" @endif>
                ...
                <td>
                  <input type="text" name="amount[]" class="form-control" placeholder="Amount (Deposit/Withdrawl)" value="{{$txn->amount}}" @if(!empty($txn->is_month_payout_generate))  @else disabled="disabled" @endif>
                </td>
                <td>
                   <select class="form-control" name="payment_type[]" onChange="payment(this)" @if(!empty($txn->is_month_payout_generate))  @else disabled="disabled" @endif>
                      <option value="cash" @if($txn->payment_type == 'cash') selected="selected" @endif>Cash</option>
                      <option value="cheque" @if($txn->payment_type == 'cheque') selected="selected" @endif>Cheque</option>
                      <option value="dd" @if($txn->payment_type == 'dd') selected="selected" @endif>DD</option>
                      <option value="NEFT" @if($txn->payment_type == 'NEFT') selected="selected" @endif>NEFT</option>
                      <option value="RTGS" @if($txn->payment_type == 'RTGS') selected="selected" @endif>RTGS</option>
                   </select>
                </td>
                @if($txn->payment_type != 'cash')
                    <td style="" class="payment_mode">
                        <input type="hidden" name="bank_id_old[]" value="{{ $txn->bank_id }}">
                        <select name="bank_id[]" class="form-control" @if(!empty($txn->is_month_payout_generate))  @else disabled="disabled" @endif>
                            <option value="">Select Bank Name</option>
                            @foreach($banks as $bank)
                            <option value="{{$bank->id}}"@if($txn->bank_id == $bank->id) selected="selected" @endif>{{$bank->bank_name}}</option>
                            @endforeach
                        </select>
                        &nbsp;<input type="text" class="form-control"  name="cheque_dd_number[]" id="cheque_dd_number" placeholder="Cheque/DD Number" value="{{$txn->cheque_dd_number}}" @if(!empty($txn->is_month_payout_generate))  @else disabled="disabled" @endif>&nbsp;<input type="hidden" class="form-control"  name="date[]" id="date" placeholder="Enter date" data-toggle="tooltip" title="Cheque/DD Date" value="{{$txn->cheque_dd_date}}" @if(!empty($txn->is_month_payout_generate))  @else disabled="disabled" @endif>
                    </td>
                @endif
                <td>
                  <textarea name="remarks[]" rows="1" class="form-control" placeholder="Remarks..." @if(!empty($txn->is_month_payout_generate))  @else disabled="disabled" @endif>{{$txn->remarks}}</textarea>
                </td>
                <td>
                  <a class="@if(!empty($txn->is_month_payout_generate)) remove_field @else disabled @endif btn text-danger pt-2 mt-1" data-id="{{$txn->id}},{{$txn->associate_id}},{{$txn->respective_table_id}}" data-toggle="tooltip" title="{{ucwords($txn->transaction_type)}} Delete"><i class="material-icons">delete</i></a>
                </td>
              </tr>
              ...
              <tr>
                ...
                <td>
                  <a @if(!empty($txn->is_month_payout_generate)) href="{{route('admin.asso_delete_all_bulktxn',$params)}}" @endif  class="btn btn-danger" style="margin-top: 15px;" onclick="return confirm('Are You Sure')">Delete All</a>
                </td>
              </tr>
              @endif
            @endforeach
