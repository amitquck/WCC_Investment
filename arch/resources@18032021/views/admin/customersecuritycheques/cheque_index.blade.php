          ...
          @if($security_cheque->count()>0)
            @foreach($security_cheque as $key => $cheque)
              @if($cheque->cheque_issue_date)
                <tr>
                  <td>{{$key+1}}.</td>
                  <td class="text-info"><a href="{{route('admin.customer_detail',encrypt($cheque->cheque_user->id))}}" data-toggle="tooltip" title="View Details">{{$cheque->cheque_user->code}}</a></td>
                  ...
                  <td>₹ {{$cheque->cheque_amount?$cheque->cheque_amount:'N/A'}}</td>
                  <td>
