...
class ReportController extends Controller
{
    ...
    public function getPageAssociateWiseCustomer(Request $request){
        // dd($request->all());
        $associate_txn = AssociateTransactions::where('associate_id', $request->id)->groupBy('customer_id')->paginate(20);
    ...
    {
        // dd($request->all());
        $associate_txn = AssociateTransactions::where('customer_id',$request->customer_id)->where('associate_id',$request->associate_id)->whereBetween('deposit_date',[date($request->year.'-'.$request->month.'-'.'01' ),date($request->year.'-'.$request->month.'-'.'t' )])->paginate(20);
        // dd($associate_txn);
    ...
    public function monthly_payout(Request $request)
    {
        ...
        $cust_reward = '';
        if($request->has('month') && $request->input('month')!= '' && $request->has('year') && $request->input('year')!= ''){
            $cust_reward = CustomerReward::where('month',$month)->where('year',$request->year)->groupBy(['customer_id','month','year'])->paginate(50);
        }else{
