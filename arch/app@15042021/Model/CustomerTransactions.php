...
class CustomerTransactions extends Model
{
    ...
    public function getCustomerWiseLastMonthBusinessAttribute()
    {
        ...
        // $cust_reward = CustomerReward::where('customer_id',$this->customer_id)->where('reward_type','interest')->where('month',date('m',strtotime(date('Y-m-01').'-1 month')))->where('year',date('Y',strtotime(date('Y-m-01').'-1 month')))->sum('amount');

        // $asso_reward = AssociateReward::where('customer_id',$this->customer_id)->where('reward_type','commission')->where('month',date('m',strtotime(date('Y-m-01').'-1 month')))->where('year',date('Y',strtotime(date('Y-m-01').'-1 month')))->sum('amount');
