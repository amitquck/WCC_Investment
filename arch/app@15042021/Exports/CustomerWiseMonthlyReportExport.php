    ...
    */
    public function collection()
    {
      ...
      if($this->month && $this->year != ''){
        ...
        $rewards = $user->customerrewards()->where('month',$month)->where('year',$this->year)->groupBy(DB::Raw('CONCAT(year,month)'))->get();
        foreach($rewards as $reward){
          $monthlyReport[$this->year.'-'.$month]['interest_amount'] = $reward->sum_monthly_interest;
          $monthlyReport[$this->year.'-'.$month]['customer'] = $reward->customer;
          $monthlyReport[$this->year.'-'.$month]['commissions'] = $user->customerassociaterewards()->where('month',$month)->where('year',$this->year)->whereCustomerId($this->id)->groupBy(DB::Raw('CONCAT(year,month)'),'associate_id')->get();
        }
      }else{
    ...
    //         foreach($monthlyReport as $key => $monthd){
	   ...
	   //    	}
        ...
        $activity_log->statement = 'Customer Wise Monthly Report Excel Export By '.auth()->user()->name.' Since At '.date('d-m-Y');  
