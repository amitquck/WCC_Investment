				...
				if($request->cheque_issue_date){
					foreach($request->cheque_issue_date as $key => $issue_date){
						...
						$customer->customersecuritycheque()->create([
							...
							'cheque_maturity_date' => date('Y-m-d',strtotime($request->cheque_maturity_date[$key])),
							...
							'cheque_date' => date('Y-m-d',strtotime($request->cheque_date[$key])),
	...
	public function update_bulk_txn(Request $request)
	{
		...
		// dd($customerId);
			...
			if($request->transaction_type[$key] == 'deposit'){
				...
				if($customerId != NULL){
					$deposit = CustomerTransactions::where('customer_id', $customerId)->where('transaction_type', 'withdraw')->first();
					// dump($customerId);
					// dd($request->customer_id[$key]);
					$deposit->customer_id = $request->customer_id[$key];
					$deposit->amount = $request->amount[$key];
					$deposit->cr_dr = 'dr';
					$deposit->payment_type = $request->payment_type[$key];
