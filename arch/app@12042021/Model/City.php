...
class City extends Model
{
    ...
    public function getCityWiseThisMonthBusinessAttribute()
    {
        ...
        $txn = CustomerTransactions::whereIn('customer_id',function($q){
            return $q->select('customer_id')->where('city_id',$this->id)->from('customer_details');
        })->where('cr_dr','cr')->where('transaction_type','deposit')->whereBetween('deposit_date',[date('Y-m-01'),date('Y-m-t')])->sum('amount');
    ...
    public function getCityWiseLastMonthBusinessAttribute()
    {
        ...
        $txn = CustomerTransactions::whereIn('customer_id',function($q){
            return $q->select('customer_id')->where('city_id',$this->id)->from('customer_details');
        })->where('cr_dr','cr')->where('transaction_type','deposit')->whereBetween('deposit_date',[date('Y-m-01',strtotime(date('Y-m-01').'-1 months')),date('Y-m-t',strtotime(date('Y-m-01').'-1 months'))])->sum('amount');
