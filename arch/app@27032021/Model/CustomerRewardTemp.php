...
class CustomerRewardTemp extends Model
{
    ...
    protected $fillable = ['customer_id','amount','commission_percent','month','year','start_date','end_date','total_amount','commission_percent','reward_type','payable_days'];
    ...
    public function getSumInterestAmountAttribute()
    {
        return CustomerRewardTemp::where('customer_id',$this->customer_id)->where('month',$this->month)->where('year',$this->year)->where('reward_type','interest')->sum('amount');
    }

    public function getSumCommissionAmountAttribute()
    {
        return AssociateRewardTemp::where('associate_id',$this->associate_id)->where('month',$this->month)->where('year',$this->year)->where('reward_type','commission')->sum('amount');
    }
