...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $top_associates = DirectAssociateCommission::groupBy('associate_id')->orderByRaw('SUM(total_investment) DESC')->limit(5)->get();
        ...
        $top_invest = CustomerTransactions::where('cr_dr','cr')->where('transaction_type','deposit')->groupBy('customer_id')->orderByRaw('SUM(amount) DESC')->limit(5)->get();
        ...
        $last_month_deposit = CustomerTransactions::where('cr_dr','cr')->where('transaction_type','deposit')->whereBetween('deposit_date',[date('Y-m-01',strtotime('-1 month')),date('Y-m-t',strtotime('-1 month'))])->sum('amount');
        // dd($last_month_deposit);

        $last_month_withdraw = CustomerTransactions::where('cr_dr','dr')->where('transaction_type','withdraw')->whereBetween('deposit_date',[date('Y-m-01',strtotime('-1 month')),date('Y-m-t',strtotime('-1 month'))])->sum('amount');

        $last_month_comm = AssociateReward::where('month',date('m',strtotime('-1 month')))->where('year',date('Y',strtotime('-1 month')))->sum('amount');
        $last_month_inter = CustomerReward::where('month',date('m',strtotime('-1 month')))->where('year',date('Y',strtotime('-1 month')))->sum('amount');
        // dd($last_month_inter);
        ...
        return view('admin.dashboard',compact('total_customer','total_deposit','total_interest','total_associate_commission','top_associates','current_invest','top_invest','data','customerdata','associatedata','total_associate_customer','this_month_txn','this_month_comm','this_month_inter','last_month_comm','last_month_inter','total_associate','total_withdraw','last_month_deposit','last_month_withdraw'));
    }
