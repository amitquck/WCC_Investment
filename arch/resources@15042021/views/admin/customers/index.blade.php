            ...
            <tr>
              ...
              <td>
                @if(empcan('payment_status_customer') || Auth::user()->login_type == 'superadmin')
                  <a class="btn  text-success hold_remarks @if($user->hold_status)  @else text-danger @endif" data-toggle="tooltip" data-id="{{$user->id}}" title="@if($user->hold_status) Make Hold Payment @else Make Pay @endif">@if($user->hold_status)<i class="material-icons">thumb_up_alt</i> @else <i class="material-icons">thumb_down_alt</i> @endif
                  </a>
                @endif
              </td>
              <td class="table-actions">
                <div class="dropdown">
                    ...
                    @if(empcan('view_transaction_customer') || Auth::user()->login_type == 'superadmin')
                      <a href="{{route('admin.customerAllTransactions',encrypt($user->id))}}" class="btn btn-default btn-link btn-icon bigger-130 text-warning viewAssociate" data-toggle="tooltip" title="View Transactions"  data-id="{{$user->id}}"><i class="material-icons">visibility</i></a>
                    @endif
                    ...
                    @if(empcan('add_withdraw_customer') || Auth::user()->login_type == 'superadmin')
                      <a class="btn  btn-default bigger-130 text-danger @if($user->customerdetails->payment_type == 'hold' || $user->customer_current_balance() <= 0) disabled @elseif($user->hold_status) customerWithdrawForm @else disabled @endif " data-toggle="tooltip" title="Add Withdraw"  data-id="{{$user->id}}"><i class="material-icons">account_balance_wallet</i></a>   
                    @endif   
