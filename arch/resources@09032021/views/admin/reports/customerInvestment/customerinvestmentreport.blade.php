          ...
          @isset($customer_deposits)
            @if($customer_deposits->count()>0)
              @foreach($customer_deposits as $key => $deposit)
                <tr>
                  <td>{{ ($customer_deposits->currentpage()-1) * $customer_deposits->perpage() + $key + 1 }}. </td>
                  <td>{{$deposit->user->name}} <br>
                    {{$deposit->user->code}}</td>
                  ...
                  <td>
                    @if($deposit->payment_type == 'cash' || $deposit->payment_type == 'null')
                      {{'Cash Payment'}}
                    @else
                      <strong>Bank Name : </strong>{{$deposit->bankname?$deposit->bankname->bank_name:'N/A'}}
                    <br>
                      <strong>Cheque/DD Number : </strong>{{$deposit->cheque_dd_number?$deposit->cheque_dd_number:'N/A'}}
                    @endif
                  </td>
                  <td>{{Carbon\Carbon::parse($deposit->deposit_date)->format('j-m-Y')}}</td>
                  <td>
                  {{Carbon\Carbon::parse($deposit->created_at)->format('j-m-Y')}}</td>
                  <td>
...
@php
  ...
  if(Request::has('to_date'))
    $params = array_merge(['to_date' => Request::input('to_date')],$params);
  @endphp
{{$customer_deposits->appends($params)->links()}}
@endsection
