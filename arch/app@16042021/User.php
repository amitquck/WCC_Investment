...
class User extends Authenticatable
{
    ...
    public function customerMainBalance(){
        $totalcr = $this->customertransactions->where('transaction_type','deposit')->sum('amount');
        $totaldr = $this->customertransactions->where('transaction_type','withdraw')->sum('amount');
        return $totalcr-$totaldr;
    }
    
    public function customerLastMonthMainBalance(){
        $totalcr = $this->customertransactions->where('transaction_type','deposit')->whereBetween('deposit_date',[date('Y-m-01',strtotime(date('Y-m-01').'-1 months')),date('Y-m-t',strtotime(date('Y-m-01').'-1 months'))])->sum('amount');
        $totaldr = $this->customertransactions->where('transaction_type','withdraw')->whereBetween('deposit_date',[date('Y-m-01',strtotime(date('Y-m-01').'-1 months')),date('Y-m-t',strtotime(date('Y-m-01').'-1 months'))])->sum('amount');
        return $totalcr-$totaldr;
    }
    
    public function customerThisMonthMainBalance(){
        $totalcr = $this->customertransactions->where('transaction_type','deposit')->whereBetween('deposit_date',[date('Y-m-01'),date('Y-m-t')])->sum('amount');
        $totaldr = $this->customertransactions->where('transaction_type','withdraw')->whereBetween('deposit_date',[date('Y-m-01'),date('Y-m-t')])->sum('amount');
        return $totalcr-$totaldr;
    }
