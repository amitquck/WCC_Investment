...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $top_associates = AssociateReward::groupBy('associate_id')->orderByRaw('SUM(amount) DESC')->limit(5)->get();
        // dd($top_associates);
        ...
        $last_month_txn = CustomerTransactions::where('cr_dr','cr')->where('transaction_type','deposit')->whereBetween('deposit_date',[date('Y-m-01',strtotime('-1 month')),date('Y-m-t',strtotime('-1 month'))])->sum('amount');
        ...
        // $c = implode(',',$var);
        // dd($data);
        return view('admin.dashboard',compact('total_customer','total_deposit','total_interest','total_associate_commission','top_associates','current_invest','top_invest','data','customerdata','associatedata','total_associate_customer','this_month_txn','this_month_comm','this_month_inter','last_month_txn','last_month_comm','last_month_inter'));
    }
