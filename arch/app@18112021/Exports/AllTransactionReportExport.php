...
class AllTransactionReportExport implements FromCollection
{
    ...
    */
    public function collection()
    {
    	...
    	$records[] = ['Customer/Associate Name', 'Code','Credit','Debit', 'Transaction Type', 'Payment Mode', 'Bank Name', 'Cheque/DD Number', 'Deposit Date/Withdrawl Date', 'remarks','Created At'];
    	...
    	if($this->from_date && $this->to_date || $this->payment_type != ''){
            ...
            // dd('payment_type');
                $Associatetransactions->where('payment_type', $this->payment_type);
            }
        }
        ...
        if($this->customer != ''){
        	$transactions->whereIn('customer_id',function($q){
                return $q->select('id')->where('code','like','%'.$this->customer.'%')->orWhere('name','like','%'.$this->customer.'%')->where('login_type','customer')->from('users');
            });
	        ...
	        $activity_log->statement = 'All Transaction Report Excel Export By '.auth()->user()->name.' Since At '.date('d-m-Y');
			...
			foreach($db_records as $key => $record){//$transaction->associate->name//customers
				if($record->associate != NULL || $record->customers != NULL){
                    ...
                    $records[$key]['Cheque/DD Number'] = $record->cheque_dd_number?$record->cheque_dd_number:'N/A';
                    $records[$key]['Deposit Date/Withdrawl Date'] = $record->deposit_date;
                    $records[$key]['remarks'] = $record->remarks;
                    $records[$key]['Created At'] = $record->created_at;
                }
	      	}
	      	$activity_log = new ActivityLog;
