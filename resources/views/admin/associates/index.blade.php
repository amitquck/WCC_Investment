...
@section('content')
  ...
  @php
    $params = [];
    if(Request::has('associate_code'))
         $params = array_merge(['associate_code' => Request::input('associate_code')],$params);
    if(Request::has('associate_name'))
         $params = array_merge(['associate_name' => Request::input('associate_name')],$params);
    if(Request::has('associate_mobile'))
         $params = array_merge(['associate_mobile' => Request::input('associate_mobile')],$params);
    if(Request::has('associate_bank_account'))
         $params = array_merge(['associate_bank_account' => Request::input('associate_bank_account')],$params);
  @endphp
  <div class="col-md-12">
    ...
    <form action="{{route('admin.associate_search')}}" method="get">
      <div class="row">
        ...
        @if(empCan('export_associate') || Auth::user()->login_type == 'superadmin')
          <div class="col-md-1">
            <a href="{{route('admin.all_associate_list_excel_export',$params)}}" class="btn btn-success btn-md" data-toggle="tooltip" title="Excel Export"><i class="material-icons">import_export</i>
              </a>
          </div>
        @endif
      </div>
              ...
              <tr>
                ...
                <td class="text-danger">
                  @if($user->associatebalance() > 0)
                    <a href="{{route('admin.list_associate_ladger',['associate'=>$user->code])}}" class="text-success" data-toggle="tooltip" title="View Ladger Balance">₹ {{$user->associatebalance()}}</a>
                  @else
                    <a href="{{route('admin.list_associate_ladger',['associate'=>$user->code])}}" class="text-danger" data-toggle="tooltip" title="View Ladger Balance">₹ {{$user->associatebalance()}}</a>
                  @endif
                </td>
                <td>
                ...
                <div class="dropdown">
                    ...
                    @if(empCan('add_transaction_associate')|| Auth::user()->login_type == 'superadmin')
                      <a data-id="{{$user->id}}" class="btn btn-default btn-link btn-icon bigger-130 text-primary @if($user->associatebalance() <= 0 || $user->lock_status == 1) disabled @else AddTransationForm @endif" data-toggle="tooltip" data-placement="top" title="Add Transaction"><i class="material-icons" style="color:#fc7b03">payments</i></a>
                    @endif
