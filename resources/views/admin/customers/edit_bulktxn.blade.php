            ...
            <thead>
              <tr>
                <th><label>Deposit Date <sup class="text-danger">*</sup></label>
                  <input type="" name="deposit_date" class="form-control" value="@if($params) <?php echo date('j-m-Y',strtotime($params['deposit_date'])) ?> @endif">
                </th>
                <th>
          ...
          @if($customer_txn)
            @foreach($customer_txn as $key => $txn)
                <tr>
                    ...
                    <input type="hidden" name="customer_id_old[]"  value="{{$txn->customer_id}}">
                    <input type="hidden" name="customer_id[]" id="hidden_cust_search" class="hidden_cust_search"  value="">
                    </td>
                    <td>
                ...
                <tr>
                    ...
                    <td>
                    <a @if(!$txn->is_month_payout_generate) href="{{route('admin.delete_all_bulktxn',$params)}}" @endif style="margin-top: 15px;" class="btn btn-danger" onclick="return confirm('If payout generated of any customer than can not be deleted. Are You Sure.')">Delete All</a>
                    </td>
                </tr>
                @endif
            @endforeach
