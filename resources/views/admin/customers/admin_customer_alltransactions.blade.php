            ...
            @if($transactions->count()>0)
              @foreach($transactions as $key => $transaction)
                <tr>
                  ...
                  <td>{{$transaction->posting_date?Carbon\Carbon::parse($transaction->posting_date)->format('j-m-Y'):Carbon\Carbon::parse($transaction->deposit_date)->format('j-m-Y')}}</td>
                  ...
                  <td>
                    @if(Auth::user()->login_type == 'superadmin' && $transaction->transaction_type == 'deposit')
                      @if(empcan('edit_transaction_customer') || Auth::user()->login_type == 'superadmin' && $transaction->transaction_type == 'deposit')
                      ...
                      @endif
                      @if(empcan('delete_transaction_customer') || Auth::user()->login_type == 'superadmin' && $transaction->transaction_type == 'deposit')
                    ...
                    @elseif(Auth::user()->login_type == 'superadmin' && $transaction->transaction_type == 'withdraw')
                      @if(empcan('edit_transaction_customer') || Auth::user()->login_type == 'superadmin' && $transaction->transaction_type == 'withdraw')
                      ...
                      @endif
                      @if(empcan('delete_transaction_customer') || Auth::user()->login_type == 'superadmin' && $transaction->transaction_type == 'withdraw')
                        <a class="btn btn-link btn-icon bigger-130 text-danger @if(!$transaction->is_month_payout_generate) delete_confirm @else disabled @endif"  data-id="{{$transaction->customer_id}},{{$transaction->id}}"><i class="material-icons" data-toggle="tooltip" title="Delete" data-placement="top">delete_outline</i></a>
                      @endif
                    @endif
                  </td>
