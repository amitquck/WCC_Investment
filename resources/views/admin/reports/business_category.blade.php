      ...
      <table class="table table-nostretch table-align-middle mb-0" id="myTable">
        ...
        <tbody>
          ...
          @elseif($q == 'state')
            @if($cust_txn->count()>0)
              @foreach($cust_txn as $key => $txn)
                @if($txn->getState != ''  && $txn->cust_balance != 0)
                  <tr>
                    <td>{{($cust_txn->currentpage()-1)*$cust_txn->perpage()+$key+1}}.</td>
              ...
              @foreach($cities as $key => $city)
                @if($city->getCity != '' && $city->cust_balance != 0)
                  ...
                  <tr>
                    ...
                    <td>{{ getStateName($city->city_id) }}</td>
                    <td>{{ucwords($city->getCity->name)}}</td>
                    <td>
                      @if($city->cust_balance > 0)
                        <a href="{{route('admin.city_business',['city_id'=>$city->getCity->id, 'state_id'=>getStateId($city->city_id)])}}" data-toggle="tooltip" title="View Ladger Balance" class="text-success"> ₹ {{$city->cust_balance}}</a>
                      @else
